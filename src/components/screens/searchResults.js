...
class searchResults extends Component {
  ...
  featchSearchResult = async () => {
    fetchData(
      "https://gist.githubusercontent.com/AnkitDroidGit/24db2844429e6a90f1f8f889b6efe987/raw/b5cce14c7e1486129e3a0a35e852cdef0e6e6f25/searchResult.json"
    )
      .then(respone => {
  ...
  renderAddress = item => {
    ...
    return (
      <Text style={homeStyle.boldTextStyle} numberOfLines={1}>
        {address}
      </Text>
    );
  };
      ...
      <View style={homeStyle.searchResultcontainer}>
        <View style={homeStyle.flatListView}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={homeStyle.searchResultStyle}
                onPress={() => {
                ...
                <View style={homeStyle.searchResultStyle}>
                  <View>
                    ...
                    <View style={homeStyle.container}>
                      <Text style={homeStyle.boldTextStyle}>{item.prices == null ? "" : "RM " + item.prices[0].min}</Text>
                      {/* <Text style={homeStyle.boldTextStyle}>{item.title}</Text> */}
                      {this.renderAddress(item)}
                      <Text style={homeStyle.addressStyle}>{item.address == null ? "" : item.address.formattedAddress}</Text>
